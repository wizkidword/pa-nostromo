...
The task, reminders, email, and integrations areas remain in `public/app.js`. Extract them one feature at a time, beginning with a self-contained view and its event handlers, then move related state helpers and selectors only after behavior is verified.
