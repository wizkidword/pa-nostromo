...
function buildRouteManifest({ limits, remote } = {}) {
  ...
  const manifest = [
    ...
    read('diary.read', '/api/diary-index', { scope: 'admin', remoteAllowed: false, ratePolicy: 'filesystem-read' }),
    write('diary.refresh', '/api/diary-index/refresh', { scope: 'admin', remoteAllowed: false, bodyLimit: limits.action, ratePolicy: 'filesystem-read' }),
  ];
