...
const { validateWebUrl } = require('./url-policy.js');
...
const PROJECT_STATUSES = new Set(['planning', 'active', 'paused', 'blocked', 'done', 'archived']);
...
function validateUrl(value, path, { allowRelative = true, allowEmpty = true } = {}) {
  ...
  const validated = validateWebUrl(value, { allowRelative, allowHttp: true });
  if (!validated.ok) schemaError('invalid_url', `${path} is not a safe http(s) URL or same-origin absolute path.`, { path, reason: validated.code });
}
...
function validateKnownState(state) {
  ...
  if (state.rss != null) {
    ...
    if (state.rss.items != null) {
      if (!Array.isArray(state.rss.items) || state.rss.items.length > 10_000) {
        schemaError('invalid_collection', 'RSS items must be a bounded array.', { path: 'state.rss.items' });
      }
      state.rss.items.forEach((item, index) => {
        const path = `state.rss.items[${index}]`;
        if (!isPlainObject(item)) schemaError('invalid_collection_item', 'RSS items must be objects.', { path });
        validateId(item.id, `${path}.id`, { required: true });
        if (item.title != null) validateString(item.title, `${path}.title`, 2_000);
        if (item.summary != null) validateString(item.summary, `${path}.summary`, 8_000);
        if (item.link != null) validateUrl(item.link, `${path}.link`);
      });
    }
  }
